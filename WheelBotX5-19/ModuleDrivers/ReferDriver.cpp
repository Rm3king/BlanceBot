...
uint32_t cRef::M2M_CheckDst(uint16_t CMD)
{
	switch(CMD)
	{
		...
		case 0x0205:
			return (uint32_t)&this->robot_referee_status->ext_air_support_data;
		break;
		#endif
		...
		case 0x0208:
			#if (RF_GAME_MODE==0x01)//RMUC
			return (uint32_t)&this->robot_referee_status->ext_projectile_allowance;
			#elif (RF_GAME_MODE==0x00)//RMUL
		...
		break;		
			...
			case 0x020B:
				return (uint32_t)&this->robot_referee_status->ext_ground_robot_position;
			break;	
			#endif
			#ifdef RF_ROBOT_MARK		//0x020C
			case 0x020C:
				return (uint32_t)&this->robot_referee_status->ext_radar_mark_data;
			break;	
			#endif
		#endif
		...
		break;		
			...
			case 0x0306:
				return (uint32_t)&this->robot_referee_status->ext_custom_client_data;
			break;	
			#endif
			#ifdef RF_ITF_SENTRY		//0x0307
			case 0x0307:
				return (uint32_t)&this->robot_referee_status->ext_map_sentry_data;
			break;	
			#endif
		#endif		
		default:
										...
										uint16_t start_x, uint16_t start_y, const char *str, ...)
{
	...
	p_client_custom_character->graphic_tpye=7; 											//0-直线;1-矩形;2-整圆;3-椭圆;4-圆弧;5-浮点数;6-整形数;7-字符
	...
	p_client_custom_character->start_y=start_y; 										//起始点Y
