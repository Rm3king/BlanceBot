...
static void DebugThreadFun(ULONG initial_input)
{
	...
	for(;;)
	{
     ...
     __HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_2, serve_moto);
		...
		Txlen = DebugPrintf(pbufx,"Llen_dot1=%f,Llen_dot2=%f,",RobotControl->ChasisControl.Rleg_ObserveVal.X[1]*arm_cos_f32(RobotControl->ChasisControl.Rleg_ObserveVal.X[0]),RobotControl->ChasisControl.Rleg_ObserveVal.ChasisLenDot*arm_sin_f32(RobotControl->ChasisControl.Rleg_ObserveVal.X[0]));
