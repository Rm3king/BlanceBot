...
int main(void)
{
  ...
  /*Do not forget to enable d2 sram in system_stm32h7xx.c!!!
  
	Add thes e codes under SystemInit():
	
	__HAL_RCC_D2SRAM1_CLK_ENABLE();
	__HAL_RCC_D2SRAM2_CLK_ENABLE();
	__HAL_RCC_D2SRAM3_CLK_ENABLE();
 */
  /*Move NVIC map tp DTCM*/
  uint32_t *DestAddr = (uint32_t *)D1_DTCMRAM_BASE;
  const uint32_t *SouceAddr = (const uint32_t *)FLASH_BANK1_BASE;
